...
public class SupporterClass {
	public static ArrayList<String> ListOfUsers(String userIDRatingString) {
		...
		for (int i = 0; i < tuples.length; i++) {
			...
			String str = tuples[i].split(",")[0].substring(0);
			list.add(str);
		}
	...
	public static HashMap<String, Double> UserIDRating(String userIDRatingString) {
	...
	public static int getUsersCnt(ArrayList<String> list1, ArrayList<String> list2) {
		int count = 0;
	...
	public static HashMap<String, String> readFile(String filename) throws IOException {
		...
		if(hostName.toLowerCase().indexOf("whale") >= 0)
			filename = "hdfs:/cloudc52/" + filename;
	...
	public static double sim(HashMap<String, Double> m1, HashMap<String, Double> m2) {
		double num, denum;
	...
	public static ArrayList<String> getListOfRating(String userIDRatingString) {
		ArrayList<String> list = new ArrayList<>();
		String[] tuples = userIDRatingString.split(";");
		for (int i = 0; i < tuples.length; i++) {
			if (tuples[i].length() == 0)
			...
			String str = tuples[i].split(",")[1].substring(0);
			list.add(str);
		}
		return list;
	}
